-- ============================================================
--  CHBS2 - FINAL STORED PROCEDURES FILE (SYBASE/T-SQL)
...
BEGIN
    ...
    SELECT @is_holiday = COUNT(*) FROM CHBS2_HOLIDAYS WHERE HOLIDAYDATE = @p_MeetDate
    IF @is_holiday > 0
    BEGIN
    ...
    SELECT @is_maintenance = COUNT(*) FROM CHBS2_HALLS
    WHERE HALLID = @p_HID
        AND ISAVAILABLE = 0
    ...
    SELECT @v_isDir = ISDIRECTORHALL FROM CHBS2_HALLS WHERE HALLID = @p_HID
    
    SELECT @conflict_count = COUNT(*) FROM CHBS2_BOOKINGINFO HOLDLOCK
...
BEGIN
    ...
    SELECT @is_holiday = COUNT(*) FROM CHBS2_HOLIDAYS WHERE HOLIDAYDATE = @n_Date
    ...
    IF @p_Stat = 'CONFIRMED'
    BEGIN
        SELECT @conflict = COUNT(*) FROM CHBS2_BOOKINGINFO HOLDLOCK
...
IF OBJECT_ID('VW_CHBS2_DASHBOARD') IS NOT NULL DROP VIEW VW_CHBS2_DASHBOARD
GO
CREATE VIEW VW_CHBS2_DASHBOARD AS
SELECT 
    B.BOOKINGID,
...
IF OBJECT_ID('VW_CHBS2_HISTORY') IS NOT NULL DROP VIEW VW_CHBS2_HISTORY
GO
CREATE VIEW VW_CHBS2_HISTORY AS
SELECT 
    BH.BOOKINGID,
