...
import { Section, Step, Note, Chip } from '../components/domain/instructions/InstructionSection';
import InstructionsSidebar from '../components/domain/instructions/InstructionsSidebar';
    ...
    return (
        <div className="min-h-screen bg-[#F3F4F6] flex flex-col font-sans">
            ...
            <div className="max-w-5xl mx-auto w-full px-4 py-8 flex flex-col lg:flex-row gap-6">
                <InstructionsSidebar navItems={NAV_ITEMS} activeSection={activeSection} onNavigate={scrollTo} />
                ...
                <main className="flex-1 space-y-6">
                    ...
                    <Section id="booking" icon="📋" title="Booking via Book Hall Form" badge="Step-by-step form">
                        <div className="space-y-4">
                            <Step number={1} title="Select a Date">Dates in the past and holidays are blocked automatically.</Step>
                            <Step number={2} title="Choose a Hall">Only halls <strong>available</strong> on your selected date are shown. Halls under maintenance are automatically excluded.</Step>
                            <Step number={3} title="Select Time Slots">Pick a start time and end time from 15-minute slots. Duration is calculated automatically.</Step>
                            <Step number={4} title="Fill in Meeting Details">Enter a meeting title, select the meeting type, and specify if a video link is required.</Step>
                            <Step number={5} title="Submit the Request">
                                Click <strong>Book Hall</strong>. Director Hall bookings go into <em>Pending</em> for admin approval. Others confirm immediately if no conflict exists.
                            </Step>
                        </div>
                    ...
                    <Section id="timeline" icon="🕐" title="Timeline Page" badge="Visual drag-to-book">
                        <p className="text-sm text-slate-600">The Timeline gives you a visual, real-time grid of all halls for a selected date.</p>
                        <div className="space-y-4">
                            <Step number={1} title="Pick a Date">Use the date input at the top. The grid refreshes automatically.</Step>
                            <Step number={2} title="Drag to Select a Slot">Click and drag across empty cells. The selection snaps to 15-minute intervals and auto-stops at existing bookings.</Step>
                            <Step number={3} title="Confirm in the Pop-up">Enter the meeting title, type, and link requirement, then click <strong>Confirm Booking</strong>.</Step>
                            <Step number={4} title="Right-Click to View Details (Your Bookings)">Right-click on any booking block that belongs to <strong>you</strong> to see its full details.</Step>
                        </div>
                        ...
                        <Note icon="⚠️" color="orange">You cannot drag over existing bookings. The selection automatically clamps to the nearest free boundary.</Note>
                    </Section>

                    {/* Dashboard */}
                    <Section id="dashboard" icon="📊" title="My Dashboard" badge="Manage your bookings">
                        <p className="text-sm text-slate-600">Your dashboard shows two panels side-by-side:</p>
                        <div className="grid sm:grid-cols-2 gap-4">
                            <div className="rounded-xl border-l-4 border-[#FF6600] bg-orange-50 p-4">
                                ...
                                <p className="text-xs text-slate-600 leading-relaxed">Bookings awaiting admin approval. You can cancel at any time.</p>
                            </div>
                            <div className="rounded-xl border-l-4 border-[#007BFF] bg-blue-50 p-4">
                                ...
                                <p className="text-xs text-slate-600 leading-relaxed">All confirmed upcoming bookings and past history. Cancel a future confirmed booking here.</p>
                            </div>
                        </div>
                        <Note icon="🗑️" color="red">
                            <strong>Cancellation Policy:</strong> You can only cancel bookings with a date <strong>strictly in the future</strong> (tomorrow or later).
                        </Note>
                    </Section>

                    {/* Calendar */}
                    <Section id="calendar" icon="📆" title="Calendar Page" badge="Availability heatmap">
                        <p className="text-sm text-slate-600 leading-relaxed">The Calendar shows a monthly heatmap of booking activity. Each day is colour-coded based on hall availability.</p>
                        ...
                        <Note icon="💡" color="green">
                            Click any day in the calendar to see the booking load for that date. On a <strong>PARTIAL</strong> day, check the Timeline for exact slot availability.
                        </Note>
                    </Section>
