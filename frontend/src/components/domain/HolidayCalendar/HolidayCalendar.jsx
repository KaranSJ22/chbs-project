...
const toDateKey = (rawDate) => {
  ...
  if (rawDate instanceof Date) {
    return `${rawDate.getUTCFullYear()}-${String(rawDate.getUTCMonth() + 1).padStart(2, '0')}-${String(rawDate.getUTCDate()).padStart(2, '0')}`;
  }
...
const HolidayCalendar = () => {
  ...
  useEffect(() => {
    const loadHolidays = async () => {
      ...
      const formatted = data.reduce((acc, row) => {
        const key = toDateKey(row.HOLIDAYDATE);
        if (!key) return acc;
        acc[key] = acc[key] || [];
        acc[key].push({
  ...
  return (
    <div className="chbs-calendar-widget">
      ...
      {/* Side panel — selected date's holidays */}
